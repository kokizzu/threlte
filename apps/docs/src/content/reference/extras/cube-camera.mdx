...
Both the `renderTarget and the underlying `CubeCamera`are available
as snippet props. Usually you'll only want to use the`renderTarget.texture`.

```svelte
<CubeCamera>
  {#snippet children({ camera, renderTarget })}
    <T.Mesh>
      <T.SphereGeometry />
